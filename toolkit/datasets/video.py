...
class Video(object):
    def __init__(self, name, root, video_dir, init_rect, img_name,#img_names
        ...
        self.attr = attr
        self.pred_trajs = {}
        self.img_name = [os.path.join(root, x) for x in img_name]#img_names
        self.imgs = None
        self.img_names=[]
        if load_img:
            self.imgs = [cv2.imread(x) for x in self.img_names]
            for a in self.img_names:
                a=a.replace('/color','')
                self.img_names.append(a)

            self.width = self.imgs[0].shape[1]
            self.height = self.imgs[0].shape[0]
        else:
            for a in self.img_name:#img_names
                a=a.replace('/color','')
                self.img_names.append(a)
