...
import numpy as np
from .modules import *
from siamban.models.neck.position_encoding import build_position_encoding,PositionEmbeddingSine
...
class FCOSOcean(nn.Module):
    ...
    def forward(self, searchs, update=None):
        ...
        for idx, (search) in enumerate(zip(searchs), start=2):
            if update is None:
                 cls_x = self.cls_encode(search[0])   # [z11, z12, z13]
            else:
        ...
        search_1total = 0
        for i in range(3):
            search_1total += weight[i] * search_1chidu[i]
        search_2total = 0
        for i in range(3):
            search_2total += weight[i] * search_2chidu[i]
        search_3total = 0
        for i in range(3):
            search_3total += weight[i] * search_3chidu[i]
        #kernal_1total = 0
        #for i in range(3):
        #    kernal_1total += weight[i] * kernal_1chidu[i]
        #kernal_2total = 0
        #for i in range(3):
        #    kernal_2total += weight[i] * kernal_2chidu[i]
        #kernal_3total = 0
        #for i in range(3):
         #   kernal_3total += weight[i] * kernal_3chidu[i]
        pad2 = nn.ZeroPad2d(padding=(0, 0, 1, 1))
        #kernal_2total = pad2(kernal_2total)
        search_2total=pad2(search_2total)
        pad3=nn.ZeroPad2d(padding=(1,1,0,0))
        #kernal_3total=pad3(kernal_3total)
        search_3total=pad3(search_3total)
        #weight = F.softmax(self.weight, 0)
        #kernal_feature=weight[0]*kernal_1total+weight[1]*kernal_2total+weight[2]*kernal_3total
        weight = F.softmax(self.weight, 0)
        search_feature=weight[0]*search_1total+weight[1]*search_2total+weight[2]*search_3total
...
'''
        ...
        cls_last=weighted_avg(cls, cls_weight)
        loc_last=weighted_avg(loc, loc_weight)
        #return { 'cls': cls_last,'loc': loc_last }
        return {cls_last,  loc_last}
'''
