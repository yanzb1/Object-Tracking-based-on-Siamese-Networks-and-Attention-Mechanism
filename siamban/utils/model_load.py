...
def check_keys(model, pretrained_state_dict):
    ...
    d = [y for y in model_keys if y not in ckpt_keys]
    if d :
        print(d)
...
def load_pretrain(model, pretrained_path):
    ...
    if "state_dict" in pretrained_dict.keys():
        pretrained_dict = remove_prefix(pretrained_dict['state_dict'],
                                        'module.')
    elif "net" in pretrained_dict.keys():
        pretrained_dict = remove_prefix(pretrained_dict['net'],
                                        'module.')
        ...
        #pretrained_dict= {key: value for key, value in pretrained_dict.items() if (key in model_dict and 'head' not in key)}
    else:
