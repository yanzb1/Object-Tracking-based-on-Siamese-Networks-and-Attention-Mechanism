...
def train(train_loader, model, optimizer, lr_scheduler, tb_writer):
    ...
    for idx, data in enumerate(train_loader):
        ...
        loss_dict = objective(outputs, targets)
        weight_dict = objective.weight_dict
...
def main():
    ...
    if cfg.BACKBONE.PRETRAINED:
       # cur_path = os.path.dirname(os.path.realpath(_ _file__))
        #backbone_path = os.path.join(cur_path, '../', cfg.BACKBONE.PRETRAINED)
        backbone_path='../backbone.pth'
        load_pretrain(model.backbone, backbone_path)
        neck_path='../true_neck.pth'
        load_pretrain(model.neck,neck_path)
